...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      Promise[T]().complete(Success(value)).future
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]
      fs map ( f0 => p.tryCompleteWith(f0))
      p.future
    }
    ...
    def delay(t: Duration): Future[Unit] = {
      Future{Await.ready(never, t) onComplete { case _ => Unit}}
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = 
      Try( Await.result(f, 0 nanos )) match {
        case Success(t) => t
        case failure @ Failure(e) => throw new NoSuchElementException
    }
