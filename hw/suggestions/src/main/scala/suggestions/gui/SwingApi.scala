...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    /** Returns a stream of text field values entered in the given text field.
     ...
     def textValues: Observable[String] = 
      Observable( observer => field subscribe {
        case ValueChanged(tf) => if (!observer.isUnsubscribed) observer.onNext(tf.text)
      }) 
  }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = 
      Observable( observer => button subscribe {
        case ButtonClicked(b) => if (!observer.isUnsubscribed) observer.onNext(b)
       })
  }
